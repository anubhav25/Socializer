...
app.post('/changePassword', (req, res) => {
    ...
    Login.findOne({ username: req.body.username }, (err, user) => {
        if (err) {
            ...
            user.changePassword(req.body.oldpassword, req.body.newpassword, (err, newuser) => {
                if (err) {
                    res.json({ response: 'fail', message: err.message })
                } else {
        ...
        if (err) {
            ...
            if (req.file) {
                ...
                thumb({
                    ...
                    destination: 'C:\\Users\\ANUBHAV\\Desktop\\New folder\\Angular\\Socializer\\src\\assets\\thumbnails\\',
                    concurrency: 1,
                    basename: nameOfFile,
                    suffix: '',
                    width: 300
            ...
            req.body.thumbnail = req.body.imglink.replace('profilePictures', 'thumbnails')
...
app.get('/search/:query', (req, res) => {
    ...
    query.exec(function(err, users) {
        if (!err) {
            var query = UserRequest.find({ username: regex }, { 'username': 1, 'thumbnail': 1 }).limit(5);
...
app.get('/user/:username', (req, res) => {
    ...
    var query = User.findOne({ username: req.params.username }, (err, user) => {
        if (err) {
            ...
            if (!user) {
                var query = UserRequest.findOne({ username: req.params.username }, (err, users2) => {
                    if (!err) {
                        res.json(users2);
                    } else {
                        res.json(err);
